        ...
        <div className="glass-card rounded-[2rem] p-6 sm:p-8 border border-white/80 shadow-[0_4px_24px_rgba(30,35,60,0.03)] flex flex-col lg:flex-row gap-8 lg:items-center justify-between relative overflow-hidden bg-gradient-to-br from-white to-[#F8F9FC]">
          ...
          <div className="flex flex-col gap-4 relative z-10 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-2 sm:gap-3 w-full lg:w-auto">
              <button className="flex-1 lg:flex-none px-5 py-2.5 rounded-xl bg-white text-[#1F2430] text-[14px] font-bold shadow-sm border border-[#1F2430]/10 hover:bg-[#F8F9FC] transition-colors flex items-center justify-center gap-2">
                <Share2 className="w-4 h-4" /> Share Profile
              </button>
              <button className="flex-1 lg:flex-none px-6 py-2.5 rounded-xl bg-[#3E63F5] text-white text-[14px] font-bold shadow-[0_4px_16px_rgba(62,99,245,0.25)] hover:bg-[#2A44B0] transition-colors">
                Publish Profile
              </button>
              {/* Export Overflow */}
              <button
                className="p-2.5 rounded-xl bg-white text-[#1F2430]/40 border border-[#1F2430]/10 hover:bg-[#F8F9FC] hover:text-[#1F2430] transition-colors flex items-center justify-center shadow-sm shrink-0"
                title="More Options"
              >
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <AnimatedContent direction="vertical" distance={20} delay={0.2}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {PROFILE_DATA.top_strengths.slice(0, 3).map((strength, i) => (
                <div key={i} className="p-6 rounded-[1.5rem] bg-white border border-[#1F2430]/[0.06] shadow-sm flex flex-col gap-4 hover:shadow-[0_8px_24px_rgba(30,35,60,0.04)] transition-all relative overflow-hidden">
                  ...
                  <h4 className="font-[Manrope,sans-serif] font-extrabold text-[#1F2430] text-[18px] leading-tight z-10">{strength.title}</h4>
                  ...
                  <div className="flex flex-col gap-1.5 self-start z-10">
                    <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#10B981]/10 text-[#10B981] text-[11px] font-bold uppercase tracking-wider">
                      <Target className="w-3.5 h-3.5" /> {strength.confidence}
                    </div>
                  </div>

                  <p className="text-[14px] text-[#1F2430]/70 font-medium leading-[1.6] mt-auto pt-2 z-10">{strength.reason}</p>
                </div>
              ))}
            </div>
          </AnimatedContent>
          ...
          <AnimatedContent direction="vertical" distance={20} delay={0.25}>
            <div className="glass-card rounded-[2rem] p-8 border border-white/80 shadow-[0_4px_24px_rgba(30,35,60,0.03)] mt-2 bg-white/60">
              ...
              <div className="flex flex-wrap gap-3">
                {PROFILE_DATA.evidence_backed_skills.map((skill, i) => {
          ...
          <AnimatedContent direction="vertical" distance={20} delay={0.25}>
            <div className="glass-card rounded-[2rem] p-8 border border-white/80 shadow-[0_4px_24px_rgba(30,35,60,0.03)] mt-2">
              ...
              <div className="space-y-6">
                {PROFILE_DATA.evidence_highlights.map((item, i) => (
                  <div key={i} className="flex gap-5">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-2">
                        <h4 className="font-[Manrope,sans-serif] font-bold text-[#1F2430] text-[16px]">{item.skill}</h4>
                        <span className="text-[12px] font-bold text-[#1F2430]/40 px-2 py-0.5 rounded-md bg-[#1F2430]/5">{item.session}</span>
                      </div>
          ...
          <AnimatedContent direction="vertical" distance={20} delay={0.3}>
            <div className="glass-card rounded-[2rem] p-8 border border-white/80 shadow-[0_4px_24px_rgba(30,35,60,0.03)] mt-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-10">
                {Object.entries(PROFILE_DATA.work_style).map(([key, value]) => (
                  <div key={key}>
                    <div className="text-[13px] font-extrabold text-[#1F2430]/40 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                      <Fingerprint className="w-3.5 h-3.5" /> {key.replace('_', ' ')}
                    </div>
                    <p className="text-[15px] text-[#1F2430]/80 font-medium leading-relaxed">{value}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <AnimatedContent direction="vertical" distance={20} delay={0.15}>
            <div className="glass-card rounded-[2rem] p-6 border border-white/80 bg-white/60 shadow-[0_4px_24px_rgba(30,35,60,0.03)] relative overflow-hidden flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-2 mt-1">
                 <button className="py-2.5 rounded-xl bg-white text-[#1F2430] text-[13px] font-bold shadow-sm border border-[#1F2430]/10 hover:bg-[#F8F9FC] transition-colors">
                  Share
                 </button>
                 <button className="py-2.5 rounded-xl bg-[#3E63F5] text-white text-[13px] font-bold shadow-sm hover:bg-[#2A44B0] transition-colors">
                  Publish
                 </button>
              </div>
          ...
          <AnimatedContent direction="vertical" distance={20} delay={0.25}>
            <div className="glass-card rounded-[2rem] p-6 border border-[#F59E0B]/20 bg-gradient-to-b from-[#F59E0B]/5 to-transparent">
              ...
              <div className="space-y-5">
                {PROFILE_DATA.skills_to_strengthen.map((skill, i) => (
                  <div key={i} className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <h4 className="font-[Manrope,sans-serif] font-bold text-[#1F2430] text-[14px]">{skill.title}</h4>
                      <span className="text-[10px] font-extrabold uppercase tracking-wide bg-[#F59E0B]/10 text-[#F59E0B] px-2 py-0.5 rounded">
                        {skill.label}
                      </span>
                    </div>
                    <p className="text-[13px] text-[#1F2430]/60 font-medium leading-relaxed">{skill.note}</p>
                  </div>
                ))}
              </div>
            </div>
